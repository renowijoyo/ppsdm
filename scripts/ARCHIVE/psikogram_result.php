...
$psikotes_con = mysqli_connect('ppsdm.com', 'ppsdm', 'ppsdM2014','psikotes') or die("Unable to connect to MySQL");
...
//	echo $que;	
		...
		//	echo $row['scores_string']. "<br/><br/>";
			
				$no_pemeriksaan = substr($delivery_result,strrpos($delivery_result,'#'));
					$nama = $row['ppsdm_profile'];
					$ttl = "15 April 1997";
					$jenis_kelamin = "Perempuan";
					$pendidikan = "SMA";
					$tgl_pemeriksaan = "7 Juli 2014";
					$tujuan_pemeriksaan = $row['tao_delivery_label'];
					
					$scores = json_decode($row['scores_string']);
					
					foreach ( $scores as $key => $object)
					{
						...
						foreach ( $object as $key2 => $value)
							...
							//	echo $key2 . " : " . $value;
								//	echo "<br/><br/>";
									array_push($truefalse_array,$value);
									$truefalse_string .= $value . ',';
									if ($value > 0) {
										$true_count++;
									} else if ($value < 0)
									{
					...
					//$explode_temp = explode("/",$row["tao_truefalse_string"]);
					//$true = $explode_temp[0];
					//$false = $explode_temp[1];
//					$false = explode('/',$row['tao_truefalse_string'])[1];
					//$unscaled = round($true/($true+$false) * 20);
					//$truefalse_array[$row['subtest_label']] = explode('/', $row['tao_truefalse_string']);

			}

			mysqli_close($psikotes_con);
			
			//print_r($truefalse_array);
			echo "<br/>" . $truefalse_string . "<br/>";
			echo "<br/>true : " . $true_count. "<br/>";
			echo "<br/>false : " . $false_count. "<br/>";
			echo "<br/>unanswered : " . $unanswered_count. "<br/>";
...
$score_array['pengetahuan_umum'] = $con->scale('20to7',$con->scale('7_info',$truefalse_array['subtest 7'][0]));
$score_array['daya_analisa_sintesa'] = round(($con->scale('20to7',$con->scale('8_anv',$truefalse_array['subtest 8'][0])) + $con->scale('20to7',$con->scale('9_arit',$truefalse_array['subtest 9'][0])) + $con->scale('20to7',$con->scale('5_tiu',$truefalse_array['subtest 5'][0])))/3,0);
$score_array['penalaran_verbal'] = $con->scale('20to7',$con->scale('8_anv',$truefalse_array['subtest 8'][0]));
$score_array['orientasi_pandang_ruang'] = $con->scale('20to7',$con->scale('5_tiu',$truefalse_array['subtest 5'][0]));
$score_array['kemampuan_numerik'] = $con->scale('20to7',$con->scale('9_arit',$truefalse_array['subtest 9'][0]));
$score_array['klasifikasi_dan_diferensiasi'] = $con->scale('20to7',$con->scale('10_adk4',$truefalse_array['subtest 10'][0]));
$score_array['kemampuan_dasar_keteknikan'] = $con->scale('20to7',$con->scale('11_e',$truefalse_array['subtest 11'][0]));

$score_array['kecepatan_kerja'] = $con->scale2('1to7',($sum_total_dijawab / sizeof($truefalse_array)));
$score_array['ketelitian_kerja'] = $con->scale2('1to7',100*$truefalse_array['subtest 1'][0] / ($truefalse_array['subtest 1'][0] +  $truefalse_array['subtest 1'][1]));
$score_array['konsentrasi']=($score_array['ketelitian_kerja'] + $score_array['kecepatan_kerja'])/2;

$score_array['stabilitas_emosi'] = $con->scale2('130to7',130);
$score_array['hubungan_interpersonal'] = $con->scale('20to7',$con->scale('6_ist',$truefalse_array['subtest 6'][0]));
